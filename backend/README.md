...
> * 원래 설계안의 자동 반납 주기는 5분(미복귀 제한 60분)이지만, **빠른 통합 테스트 및 시연 확인을 위해 현재 `10초 주기 정기 순찰(미복귀 제한 30초)`로 조정**해 두었습니다. 
> * 실제 운영용(5분 주기), 미복귀 1시간으로 변경하려면 `app.py` 주석을 교체하시면 됩니다.
...
## 5. AI 담당자용 이벤트 연동 명세서

카메라 모듈이 특정 좌석의 사용자 이탈 및 복귀를 감지하면 백엔드 서버로 아래 데이터를 전송(Push)해 주세요.

* **URL:** `POST /internal/seats/event`
